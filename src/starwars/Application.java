...
public class Application {
	public static void main(String args[]) {
		...
		*/
		...
		//Grid controller controls the data and commands between the UI and the model
		SWUniverse universe = SWUniverse.getUniverse();
		universe.start();
		SWGridController uiController = universe.getUIController();
		Scheduler theScheduler = universe.getScheduler();
	
		// kick off the scheduler
		while(true) {
			if (theScheduler.hasLostGame()) {
				System.out.println("You have loss the game!!!");
				break;
			}
			
			if (theScheduler.hasWonGame()) {
