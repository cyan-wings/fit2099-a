...
public class SWGridController implements GridController {
	...
	public SWGridController(SWWorld world) {
		this.grid = (SWGrid) world.getGrid();
	...
	public static SWActionInterface getUserDecision(SWActor a) {
		...
		EntityManager<SWEntityInterface, SWLocation> manager;
		if (a.world() instanceof DeathStar || a.world() instanceof YavinIV){
			manager = a.world().getEntityManager();
		} else {
			manager = SWWorld.getEntitymanager();
		}
		for (SWActionInterface ac : manager.getActionsFor(a)) {
			if (ac.canDo(a))
				cmds.add(ac);
		}
